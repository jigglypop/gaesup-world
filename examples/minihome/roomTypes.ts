...
export type RoomSettings = { quality: RoomQuality; lighting: RoomLighting; camera: RoomCamera; avatar: RoomAvatarStyle; sound: RoomSound; volume: number; projection: 'orthographic' | 'perspective'; pan: boolean; rotate: boolean; damping: boolean; moveSpeed: number; bloom: boolean; bloomStrength: number; bloomRadius: number; bloomThreshold: number };
export const DEFAULT_ROOM_SETTINGS: Readonly<RoomSettings> = Object.freeze({ quality: 'balanced', lighting: 'day', camera: 'isometric', avatar: 'classic', sound: 'calm', volume: 0.35, projection: 'orthographic', pan: true, rotate: true, damping: true, moveSpeed: 3.5, bloom: true, bloomStrength: 0.6, bloomRadius: 0.4, bloomThreshold: 1.2 });
...
export type RoomOptions = { backend?: 'auto' | 'webgl'; dpr?: number; onProgress?: (stage: string) => void; onNotice?: (message: string) => void; onError?: (error: Error) => void; onCameraZoom?: (zoom: number) => void; onPaint?: (indices: number[], kind: import('./terrain').TileKind) => void; onPlace?: (kind: import('./types').FurnitureKind, x: number, z: number) => void };
export type RoomDiagnostics = {
  ...
  terrain: { tiles: number; batches: number; counts: Record<string, number> };
  visitors: number;
