...
async function metrics(ctx: ScenarioContext) {
  const objects = instances(ctx);
  const view = await mountScene(ctx, [objects.source]);
...
async function gpuMaterial(ctx: ScenarioContext) {
  ...
  try {
    ...
    for (let i = 0; i < 180 && !gpu; i++) {
      await view.frame();
      gpu = view.state.scene.getObjectByName(`gpu-resident:${objects.source.name}:0`) as Mesh | undefined;
    }
...
export const renderingScenarios: Scenario[] = [
  { id: 'metrics', title: '렌더러 계측', description: '고정된 인스턴스 장면을 렌더링하며 실제 카운터와 프레임 표본을 기록합니다.', version: 2, requirementIds: ['R14'], timed: true, run: metrics },
