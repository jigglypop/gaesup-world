...
export function useGaesupAnimation({ type }: { type: 'character' | 'vehicle' | 'airplane' }) {
  ...
  const subscribe = useCallback(
    ({ tag, condition, action, animationName, key }: AnimationAtomType) => {
      const newAnimation = {
        tag,
        condition,
        action: action || (() => {}),
  ...
  const subscribeActions = useCallback((params: Omit<subscribeActionsType, 'type'>) => {
    return useSubscribeActions({ type, ...params });
  }, [type]);

  const playActions = useCallback((params: Omit<playActionsType, 'type'>) => {
    return usePlayActions({ type, ...params });
  }, [type]);
