...
import { SaveSystem, createDefaultSaveSystem } from '../save';
...
export function createGaesupRuntime(options: GaesupRuntimeOptions = {}): GaesupRuntime {
  ...
  const gameplayEventRegistry = createDefaultGameplayEventRegistry({ dialogStore, eventsStore, inventoryStore, questStore, emit: (name, payload) => plugins.context.events.emit(name, payload) });
