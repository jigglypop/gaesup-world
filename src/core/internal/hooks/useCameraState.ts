...
export function useCameraState(blendManager: CameraBlendManager) {
  ...
  const setCameraState = useCallback(
    (stateName: string, immediate: boolean = false) => {
      ...
      if (immediate || !oldState) {
        ...
        const blendFunc = transition?.blendFunction || 'EaseInOut';
  ...
  const checkAutoTransitions = useCallback((camera: THREE.Camera, activeState: ActiveStateType) => {
    for (const transition of cameraTransitions) {
      if (transition.from === currentCameraStateName && transition.condition && transition.condition()) {
