...
const mouse = { target: new THREE.Vector3(10, 0, 0), isActive: true };
jest.mock('../useInteractionSystem', () => ({
  useInteractionSystem: () => ({ mouse }),
}));
...
describe('frame-driven player position consumers', () => {
  ...
  test('HouseDoor enters the scene when the mutable player position reaches the pad', async () => {
    ...
    const renderer = await ReactThreeTestRenderer.create(
      <HouseDoor position={[0, 0, 0]} sceneId="interior" entry={{ position: [0, 0, 0] }} radius={1} />,
    );
    try {
  ...
  test('Footsteps reads grounded/moving state at frame time, not render time', async () => {
    ...
    const renderer = await ReactThreeTestRenderer.create(<Footsteps resolveSurface={() => 'grass'} strideMeters={0.5} />);
    try {
  ...
  test('Footprints lays a print once the player walks a full step', async () => {
    const renderer = await ReactThreeTestRenderer.create(<Footprints step={0.5} />);
    try {
  ...
  test('RoomVisibilityDriver follows the player between rooms', async () => {
    ...
    const renderer = await ReactThreeTestRenderer.create(<RoomVisibilityDriver />);
    try {
  ...
  test('Clicker hides the target marker once the player arrives', async () => {
    const renderer = await ReactThreeTestRenderer.create(<Clicker />);
    const markerVisible = () => renderer.scene.findAll((node) => node.instance instanceof THREE.Group
      && (node.instance as THREE.Group).position.x === 10).length > 0;
    try {
      await renderer.advanceFrames(1, 1 / 60);
