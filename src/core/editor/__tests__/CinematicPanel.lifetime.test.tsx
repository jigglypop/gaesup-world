...
test('preview uses the owning world and stop cancels its timer and later effects', async () => {
  ...
  try {
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true })); expect(runtime.store.getState().cameraOption.focus).toBe(true);
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: '중지', exact: true })); });
...
test('custom previews receive cancellation, detach external signals on stop, and cannot publish stale completion', async () => {
  ...
  try {
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true }));
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true })); expect(signals[0]!.aborted).toBe(true); expect(signals[1]!.aborted).toBe(false); expect(removed).toHaveBeenCalled();
...
test('world disposal aborts custom preview and an inactive world cannot start another', async () => {
  ...
  try {
    ...
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true })); expect(onPreview).toHaveBeenCalledTimes(1);
...
test('external abort and errors settle previews without leaking camera state', async () => {
  ...
  try {
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true })); await act(async () => { controller.abort(); });
    ...
    fireEvent.click(screen.getByRole('button', { name: '미리 보기', exact: true })); await act(async () => { await jest.runAllTimersAsync(); });
