...
export class MemoryMonitor {
  ...
  getResult(leakThreshold: number = 1024 * 1024): MemoryTestResult {
    ...
    const growthValues = this.snapshots.slice(1).map((snapshot, index) => 
      snapshot.used - this.snapshots[index].used);
 ...
 */
export function checkThreeJSMemoryLeaks(scene: any): string[] {
  ...
  scene.traverse?.((object: any) => {
    if (object.geometry && object.geometry.dispose && !object.geometry.isDisposed) {
    ...
    if (object.material) {
      ...
      materials.forEach((material: any) => {
        if (material.dispose && !material.isDisposed) {
