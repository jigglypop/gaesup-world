import { create as g } from "zustand";
const l = [
  {
    id: "warrior-cloth-rabbit",
    name: "Rabbit Warrior Cloth",
    kind: "characterPart",
    slot: "top",
    url: "gltf/ally_cloth_rabbit.glb",
    tags: ["starter", "cloth"]
  },
  {
    id: "warrior-cloth-basic",
    name: "Basic Warrior Cloth",
    kind: "characterPart",
    slot: "top",
    url: "gltf/ally_cloth.glb",
    tags: ["starter", "cloth"]
  },
  {
    id: "ally-hat",
    name: "Ally Hat",
    kind: "characterPart",
    slot: "hat",
    url: "gltf/ally_hat.glb",
    tags: ["starter", "hat"]
  },
  {
    id: "ally-glasses",
    name: "Ally Glasses",
    kind: "characterPart",
    slot: "face",
    url: "gltf/ally_glass.glb",
    tags: ["starter", "face"]
  },
  {
    id: "starter-weapon-layer",
    name: "Starter Weapon Layer",
    kind: "weapon",
    slot: "weapon",
    url: "gltf/ally_cloth.glb",
    tags: ["starter", "weapon", "placeholder"],
    metadata: {
      placeholder: !0
    }
  },
  {
    id: "wall-soft-brick",
    name: "Soft Brick Wall",
    kind: "wall",
    tags: ["building", "wall", "brick"],
    colors: {
      primary: "#b8795f"
    },
    metadata: {
      material: "STANDARD",
      roughness: 0.72
    }
  },
  {
    id: "tile-warm-wood",
    name: "Warm Wood Tile",
    kind: "tile",
    tags: ["building", "tile", "wood"],
    colors: {
      primary: "#8b5a35"
    },
    metadata: {
      material: "STANDARD",
      roughness: 0.58
    }
  },
  {
    id: "material-glass-blue",
    name: "Blue Glass",
    kind: "material",
    tags: ["building", "material", "glass"],
    colors: {
      primary: "#9bd7ff"
    },
    metadata: {
      material: "GLASS",
      opacity: 0.45,
      transparent: !0,
      roughness: 0.1
    }
  },
  {
    id: "prop-door-basic",
    name: "Door",
    kind: "object3d",
    url: "gltf/props/door.glb",
    tags: ["starter", "building", "prop", "door", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-window-basic",
    name: "Window",
    kind: "object3d",
    url: "gltf/props/window.glb",
    tags: ["starter", "building", "prop", "window", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-fence-basic",
    name: "Fence",
    kind: "object3d",
    url: "gltf/props/fence.glb",
    tags: ["starter", "building", "prop", "fence", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-lamp-basic",
    name: "Lamp",
    kind: "object3d",
    url: "gltf/props/lamp.glb",
    tags: ["starter", "building", "prop", "lamp", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-chair-basic",
    name: "Chair",
    kind: "object3d",
    url: "gltf/props/chair.glb",
    tags: ["starter", "building", "prop", "chair", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-table-basic",
    name: "Table",
    kind: "object3d",
    url: "gltf/props/table.glb",
    tags: ["starter", "building", "prop", "table", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-bed-basic",
    name: "Bed",
    kind: "object3d",
    url: "gltf/props/bed.glb",
    tags: ["starter", "building", "prop", "bed", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-storage-basic",
    name: "Storage",
    kind: "object3d",
    url: "gltf/props/storage.glb",
    tags: ["starter", "building", "prop", "storage", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-mailbox-basic",
    name: "Mailbox",
    kind: "object3d",
    url: "gltf/props/mailbox.glb",
    tags: ["starter", "building", "prop", "mailbox", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-crafting-basic",
    name: "Crafting Station",
    kind: "object3d",
    url: "gltf/props/crafting.glb",
    tags: ["starter", "building", "prop", "crafting", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  },
  {
    id: "prop-shop-stall-basic",
    name: "Shop Stall",
    kind: "object3d",
    url: "gltf/props/shop-stall.glb",
    tags: ["starter", "building", "prop", "shop", "cc0"],
    metadata: { source: "Poly Pizza", license: "CC0" }
  }
], p = (e) => {
  const a = {};
  for (const t of e)
    a[t.id] = { ...t };
  return a;
}, u = (e, a) => a ? e.filter((t) => !(a.kind && t.kind !== a.kind || a.slot && t.slot !== a.slot || a.tag && !t.tags?.includes(a.tag))) : e, c = p(l), d = {
  state: "seed",
  origin: "seed",
  assetCount: l.length,
  loadedAt: null,
  error: null,
  fallbackReason: null
};
function i(e, a) {
  return a ? { ...e, query: { ...a } } : e;
}
const f = g((e, a) => ({
  records: c,
  ids: l.map((t) => t.id),
  isLoading: !1,
  error: null,
  catalogStatus: d,
  selectedId: null,
  filter: {},
  setFilter: (t) => e({ filter: { ...t } }),
  selectAsset: (t) => e({ selectedId: t }),
  registerAssets: (t) => e((r) => {
    const s = { ...r.records }, o = new Set(r.ids);
    for (const n of t)
      s[n.id] = { ...n }, o.add(n.id);
    return {
      records: s,
      ids: Array.from(o)
    };
  }),
  loadAssets: async (t, r) => {
    e({
      isLoading: !0,
      error: null,
      catalogStatus: i({
        state: "loading",
        origin: "source",
        assetCount: a().ids.length,
        loadedAt: null,
        error: null,
        fallbackReason: null
      }, r)
    });
    try {
      const s = await t.listAssets(r);
      if (s.length > 0) {
        a().registerAssets(s), e({
          isLoading: !1,
          catalogStatus: i({
            state: "loaded",
            origin: "source",
            assetCount: s.length,
            loadedAt: Date.now(),
            error: null,
            fallbackReason: null
          }, r)
        });
        return;
      }
      a().registerAssets(l), e({
        isLoading: !1,
        catalogStatus: i({
          state: "fallback",
          origin: "seed",
          assetCount: l.length,
          loadedAt: Date.now(),
          error: null,
          fallbackReason: "empty-source"
        }, r)
      });
    } catch (s) {
      const o = s instanceof Error ? s.message : "Failed to load assets";
      a().registerAssets(l), e({
        isLoading: !1,
        error: o,
        catalogStatus: i({
          state: "fallback",
          origin: "seed",
          assetCount: l.length,
          loadedAt: Date.now(),
          error: o,
          fallbackReason: o
        }, r)
      });
    }
  },
  getAsset: (t) => a().records[t],
  listAssets: (t) => {
    const r = a();
    return u(
      r.ids.map((s) => r.records[s]).filter((s) => !!s),
      t
    );
  },
  resetAssets: () => e({
    records: c,
    ids: l.map((t) => t.id),
    isLoading: !1,
    error: null,
    catalogStatus: d,
    selectedId: null,
    filter: {}
  })
})), m = (e) => (a) => a.listAssets({ kind: e }), h = (e) => (a) => a.listAssets({ slot: e });
export {
  l as S,
  h as a,
  m as s,
  f as u
};
