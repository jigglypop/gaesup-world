...
대표 예제는 **3D 싸이월드 스타일 미니홈피**입니다. `pnpm dev`를 실행하면 미니룸, 프로필, 다이어리, 방명록이 있는 첫 화면이 열립니다. 기본 주소는 `http://127.0.0.1:5174/`이며, 이미 사용 중인 포트라면 터미널에 표시된 주소로 접속하세요. 가구 배치와 글은 **미니홈피 저장** 버튼으로 현재 브라우저에 보관합니다. [미니홈피 사용법과 구현 범위](docs/minihome.md)를 참고하세요.

[숲 탐험·WebGPU 엔진 예제](docs/engine-showcase.md)는 `/engine`에 있습니다. 상단 **개발자 · 엔진 실험실**에서 CPU 컬링과 GPU 간접 드로우를 비교할 수 있습니다.

모듈형 아바타의 `gaesup-world/avatar` API와 에셋 계약은 [Modular Avatar v1](docs/modular-avatar.md)을 참고하세요. 미니홈피의 기본 미니미와 가구는 외부 모델 없이 생성한 도형입니다.
...
- 라이브러리 ESM 빌드: 확인 완료
- 데모 Vite 빌드: 확인 완료
- 변경 관련 Jest 테스트: 104개 통과. 전체 suite에는 누락된 에셋과 일부 기존 테스트 실패가 남아 있습니다.
- TypeScript declaration build: 통과

패키지 빌드와 외부 소비 검증은 통과했으며, 전체 검증의 남은 실패는 [예제 검증 범위](docs/engine-showcase.md#검증과-남은-범위)에 기록되어 있습니다.
