...
| 12-f | 단위 geometry 공유, world-level InstancedMesh, grass `cells` 병합, 벽 단일 재질. grass 병합 완료(2026-09-25): 타일마다 `TileObject`→`Grass`(blade·ground 메시 2개, 텍스처 로드)이던 것을 4×4 타일 chunk마다 `Grass` 하나(`cells`, 셀별 ground를 한 geometry로)로 바꿨다. 밀도(1㎡당 blade 수)는 같고 chunk key는 위치·크기·밀도·색만 써서 편집은 그 chunk만 다시 만든다. `TileObject`는 삭제. `/world?size=m`: 14.7→37.8fps, script 64.3→15.5ms, geometries 2,091→401, textures 954→98, draw 528→231. 후속(같은 날): ground는 그룹·색마다 한 geometry(LOD 없음), blade chunk는 8×8 타일로 넓혀 draw 238→144, geometries 401→187. 그룹 간 병합 완료(같은 날): `BuildingBatches`가 box 타일과 solid 벽을 재질 조합마다 world-level InstancedMesh 하나로 그리고(그룹 수와 무관), `BuildingSystem`은 그룹 System에 `batches={false}`를 넘긴다. 벽 6면 재질이 같으면 단일 재질(draw 1회). capacity는 1.5배로만 늘어 편집 중 메시 identity가 유지된다. 편집 1회는 world batch 전체 행렬을 다시 쓴다(O(타일 수)). `/world?size=m` 3회 중앙값(같은 장비, af0b99a5 대비): draw 140.4→77.6, script 5.88→3.29ms, geometries 189→101. 단위 geometry 공유(round·ramp·stair)는 잔여 | M 장면 draw call 50% 감소 |
| 12-g | water 반사 opt-in·공유, glass transmission 품질 옵션 | glass 벽 1개 추가 전후 `render.calls` 2배 증가 없음 |
| 12-h | `GpuBatchBridge` capacity 할당, 부분 업로드, geometry 참조, 재질 version 동기화. 완료(2026-09-25): storage buffer를 원본 `instanceMatrix` capacity로 잡고 cull compute의 `count`만 바꿔, 인스턴스 수 변경과 재질 version 변경에 batch(버퍼·재질 복제·커널)를 다시 만들지 않는다. 행렬은 바뀐 인스턴스 범위만 `addUpdateRange`로 올리고 bounds도 그 범위만 다시 계산한다. 재질 version이 바뀌면 생성 재질에 `needsUpdate`만 건다. world batch capacity는 처음부터 25% 여유를 둔다. geometry는 복제를 유지한다(공유 attribute를 dispose하면 렌더러가 원본 GPU 버퍼까지 지운다). `/world?size=m` 타일 5회 추가: batch 재생성 5→0, 편집 프레임 최대 간격 67~78→18~31ms | 편집 연타 중 long task 수, `info.memory.geometries` 불변 |
...
| 12-j | batch 재구성 제거(sakura, fire, flag, billboard). 완료(2026-09-25): 이 batch들은 객체 수와 무관하게 draw 수가 일정하고 프레임당 작업도 uniform 갱신뿐이라, `BuildingSystem`이 가시성 필터 없이 전체 객체로 한 번 만든다(WebGPU 경로와 같은 입력). 상주 목록 변경은 개별 React 트리인 모델만 거른다. per-instance 가시성은 WebGPU 경로의 GPU 컬링이 맡는다 | 가시성 변경 시 `computeSpecs`/`makePointsGeo` 호출 0 |
| 12-k | 에셋 미리보기 단일 renderer 썸네일 | 에셋 패널 스크롤 중 `webglcontextlost` 0 |
| 12-l | `compileAsync` 워밍업, lamp light pool. 완료(2026-09-25): `/world`는 로드 뒤 이동·카메라 회전 중 새 렌더 파이프라인 0개였지만, 램프(`fallbackKind: 'lamp'`)를 놓을 때마다 `pointLight`가 늘어 lit 재질 파이프라인 25개를 동기로 다시 만들며 2.2초 멈췄다. 램프는 발광 갓과 함께 월드의 고정 조명 풀(`LampLightPool`, 4개)을 가까운 순으로 빌리고, 풀은 첫 램프가 나타날 때 한 번만 생긴다(단독 `ModelObject`는 기존처럼 자기 조명). 처음 나타나는 객체 batch와 모델은 `CompileGate`가 `compileAsync`로 파이프라인을 만든 뒤 보여준다(three 185/186, 동기 투영 구간에만 보이게·컬링 해제). 배치 측정(`/world?size=s`): 두 번째 램프 2.1s→18ms, 빌보드 246→21ms. 첫 램프는 조명 구성이 한 번 바뀌어 1.46s가 남는다(미래 조명 구성의 선컴파일은 노드 빌드가 비동기라 현재 조명과 경합해 쓰지 않음) | warm-up 후 program 증가 0 |
