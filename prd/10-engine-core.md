...
| 스케줄러 밖 루프 | raw `useFrame` 3곳(`GpuBatchBridge`, `CascadedSun`, `WorldPostProcessing`), drei `useAnimations`(RemotePlayer), rAF 4곳(`AnimationClockLoop` 드라이버 없을 때, `BrowserGamepadHub`, 엔티티별 `useEntityLifecycle`, `PerformancePanel`), 타이머(clock 없을 때 `NetworkSystem`, autosave, BGM, minimap, ping). eslint 허용 목록에 이미 raw `useFrame`이 없는 `RigidPartRef`·`WorldPhysics`가 남아 있다 | `eslint.config.js:10-17` |
