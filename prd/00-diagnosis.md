...
| 증상 | 위치 | 비용 | 원인 | 처리 |
|---|---|---|---|---|
| NPC 기본 두뇌가 없는 정책 서버(`localhost:8091`)로 NPC마다 약 1.5초마다 요청, 실패해도 백오프 없음. 패널 마운트가 사용자가 고른 두뇌를 덮어씀 | `npc/core/reinforcement.ts:24,143`, `npc/stores/npcStore.ts:228-241,373` | NPC 50체면 약 33건/s | 기본값 결함 | DOM-03a(M0) |
