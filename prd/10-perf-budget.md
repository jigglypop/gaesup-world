...
| Slice | 내용 | 완료 기준 |
|---|---|---|
| 10-a | 기준 장면 `perf-world`(S/M/L), 궤도·hover 경로, 샘플러 추가. 현재값을 00 문서 3.2절에 기록. 완료(2026-09-25): `examples/world`(`/world?size=s|m|l`)와 `pnpm perf:world`, 값은 00 문서 3.1.3. 궤도·hover는 harness 입력 타임라인(WASD)으로 대신한다 | M 장면에서 5절 지표 전부가 JSON으로 출력 |
