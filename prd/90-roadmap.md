...
| 순위 | Slice | 이유 |
|---|---|---|
| 1 | DOM-03a | S-H11 빨강. 기본 설정에서 NPC가 없는 서버로 요청을 보낸다 |
| 2 | AST-06a | S-H10 빨강. 변경 없는 autosave가 전 도메인을 직렬화한다 |
| 3 | VER-08a, VER-08b | 깨진 probe와 내부 경로 fixture. 코어를 바꾸기 전에 회귀 장치를 살린다 |
| 4 | VER-03a, VER-04a | 운영 빌드 측정과 `/accept`. 소유자가 직접 보는 화면 |
| 5 | DOM-11a | 대표 데모가 대기 중에도 매 프레임 그린다 |
| 6 | VER-05a | 기준 장면에 NPC·원격·편집·궤도·그림자 경로가 없다 |
| 7 | COR-09c | S-H14 빨강. 엔트리 import만으로 전역 리스너 3·타이머 3·전역 쓰기 1 |
| 8 | COR-01a | 엔진 코어의 첫 단계(kernel). 이후 모든 코어 작업의 전제 |
| 9 | COR-05b | D-10. 프로젝트 설정의 고정 틱 설정이 runtime clock에 들어가지 않는다. 작고 독립적이다 |
| 10 | VER-06a | 기본 브랜치를 `main` 하나로(사용자 결정) |
