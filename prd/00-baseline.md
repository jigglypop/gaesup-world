...
| D-20 | 수정 (2026-09-25) | 남은 벽시계 임계값을 호출 수·기능 단언으로 교체(25-b). 이전 기록: | 실패가 관측된 `useBaseLifecycle` 성능 테스트를 등록/해제 호출 수 단언으로 교체. 같은 벽시계 패턴 9곳(`useBaseFrame`, `useCollisionHandler`, `useEntityLifecycle`, `advanced`, `Autowired`, `container`, `SystemRegistry`)은 10-b 잔여. 이번 검증 중에도 병렬 부하에서 원인 불명 2건이 1회 실패 후 재현되지 않음 | `useBaseLifecycle.test.ts` 26 tests 통과 |
...
| D-23 | 수정 (2026-09-24) | 공용 `rendering/useSceneToon`(layout effect 적용, cleanup 해제)을 만들고 NPC 파츠, `PhysicsEntity`, `RiderRef`가 사용. 원본 재질 키 refcount 공유(FR-12-17 후반)는 12-p 잔여 | `useSceneToon.test.tsx`(mount/unmount 100회마다 원본 복원·toon dispose 1회) |
