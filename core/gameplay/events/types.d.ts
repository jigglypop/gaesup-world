...
export type GameplayEventAction = {
    ...
    type: 'toast';
    kind?: 'info' | 'success' | 'warn' | 'error' | 'reward' | 'mail';
    text: string;
} | {
    type: 'setFlag';
    key: string;
...
export type GameplayConditionHandler<TCondition extends GameplayEventCondition = GameplayEventCondition> = (condition: TCondition, context: GameplayEventContext) => boolean | Promise<boolean>;
export type GameplayActionHandler<TAction extends GameplayEventAction = GameplayEventAction> = (action: TAction, context: GameplayEventContext) => void | Promise<void>;
