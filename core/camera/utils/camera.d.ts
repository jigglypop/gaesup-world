...
export declare function invalidateCollisionCache(): void;
...
export declare const cameraUtils: {
    ...
    smoothingToSpeed: (value: number | undefined, fallback?: number) => number;
    ...
    preventCameraJitter: (camera: THREE.Camera, targetPosition: THREE.Vector3, targetLookAt: THREE.Vector3, positionSpeed: number, deltaTime: number, rotationSpeed?: number) => void;
    improvedCollisionCheck: (from: THREE.Vector3, to: THREE.Vector3, scene: THREE.Scene, radius?: number, excludedObjects?: THREE.Object3D[]) => CollisionCheckResult;
    ...
    updateFOV: (camera: THREE.PerspectiveCamera, targetFOV: number, speed?: number, deltaTime?: number) => void;
...
export declare const activeStateUtils: {
    ...
    getCameraTarget: (activeState: ActiveStateType, cameraOption: CameraOptionType) => THREE.Vector3;
};
