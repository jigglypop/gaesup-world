...
(async () => {
  ...
  async function bind(page) {
    ...
    page.on('console', message => { if (/GPUValidationError|Invalid RenderPipeline|WebGL: INVALID/.test(message.text())) errors.push(message.text()); });
  ...
  try {
    const page = await browser.newPage({ viewport: { width: 1600, height: 1050 } }); await bind(page);
    await snapshot(page, '01-town');
    await page.getByRole('button', { name: '드로우콜·성능', exact: true }).click();
    await page.waitForFunction(() => window.miniroom.diagnostics().frame.draws.length > 0);
    const stats = await page.evaluate(() => window.miniroom.diagnostics()); result.initial = stats;
    assert.equal(stats.terrain.tiles, 576); assert.equal(stats.terrain.batches, 6);
    ...
    await clickWorld(page, [10, 0, 7]); await page.waitForFunction(() => window.miniroom.diagnostics().movement.state === 'blocked');
    ...
    await page.getByRole('button', { name: '공간 편집', exact: true }).click();
    ...
    await page.getByRole('button', { name: '편집 완료 · 둘러보기', exact: true }).click(); await clickWorld(page, [0.5, 0, 5.5]);
    ...
    await page.getByLabel('Bloom 효과', { exact: true }).uncheck();
    ...
    await page.getByLabel('Bloom 효과', { exact: true }).check();
    ...
    await page.getByLabel('미니룸 카메라', { exact: true }).selectOption('follow'); await page.getByRole('button', { name: '해변', exact: true }).click();
    await page.waitForFunction(() => window.miniroom.diagnostics().movement.state === 'arrived', null, { timeout: 15000 });
    ...
    await mobile.getByRole('button', { name: '공간 편집', exact: true }).click();
    const mobileSand = await mobile.evaluate(() => window.miniroom.diagnostics().terrain.counts.sand);
    await mobile.getByRole('button', { name: '모래 타일', exact: true }).tap(); await tapWorld(mobile, [0.5, 0, 4.5]);
    await mobile.waitForFunction(count => window.miniroom.diagnostics().terrain.counts.sand === count + 1, mobileSand);
    await mobile.getByRole('button', { name: '화분 배치', exact: true }).tap(); await tapWorld(mobile, [0.5, 0, 4.5]);
    await mobile.waitForFunction(() => window.miniroom.diagnostics().objectCount === 23);
    ...
    const roomCode = `probe-${Date.now()}`;
    await page.getByLabel('방 코드', { exact: true }).fill(roomCode); await page.getByLabel('방문 닉네임').fill('Host');
    ...
    await guest.getByLabel('방 코드', { exact: true }).fill(roomCode); await guest.getByLabel('방문 닉네임').fill('Guest');
    ...
    assert.equal(await guest.getByRole('button', { name: '방문 중 · 주인만 편집', exact: true }).isDisabled(), true);
    ...
    await clickWorld(guest, [6, 0, 6]); await guest.waitForFunction(() => window.miniroom.diagnostics().movement.state === 'arrived', null, { timeout: 15000 });
    ...
    await page.getByRole('button', { name: '공간 편집', exact: true }).click(); await page.getByLabel('미니룸 카메라', { exact: true }).selectOption('top');
    ...
    await guest.waitForFunction(() => window.miniroom.diagnostics().objectCount === 22);
