...
export var RigidBodyRef = forwardRef(function (_a, ref) {
    var children = _a.children, name = _a.name, position = _a.position, rotation = _a.rotation, userData = _a.userData, onCollisionEnter = _a.onCollisionEnter, type = _a.type, outerGroupRef = _a.outerGroupRef, innerGroupRef = _a.innerGroupRef, colliderRef = _a.colliderRef, url = _a.url, isActive = _a.isActive, currentAnimation = _a.currentAnimation, componentType = _a.componentType;
    ...
    useFrame(function () {
        if (isActive || !position || !ref || !ref.current)
            return;
        ref.current.setTranslation(V3(THREE.MathUtils.lerp(ref.current.translation().x, position.x, 0.1), THREE.MathUtils.lerp(ref.current.translation().y, position.y, 0.1), THREE.MathUtils.lerp(ref.current.translation().z, position.z, 0.1)), false);
    });
    return (_jsxs(RigidBody, { colliders: false, ref: ref, name: name, rotation: euler().set(0, (rotation === null || rotation === void 0 ? void 0 : rotation.clone().y) || 0, 0), userData: userData, onCollisionEnter: onCollisionEnter, type: type || "dynamic", children: [_jsx(CapsuleCollider, { ref: colliderRef, args: [collider.height, collider.radius], position: [0, collider.height + collider.radius, 0] }), children, _jsx(InnerGroupRef, { objectNode: objectNode, animationRef: animationRef, nodes: nodes, ref: innerGroupRef, children: children })] }));
});
