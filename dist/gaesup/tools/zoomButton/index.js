var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { jsx as _jsx } from "react/jsx-runtime";
import { useContext, useState } from "react";
import { makeNormalCameraPosition } from "../../camera/control/normal";
import { makeOrbitCameraPosition } from "../../camera/control/orbit";
import { GaesupWorldContext, GaesupWorldDispatchContext, } from "../../world/context/index";
import "./style.css";
export function useZoom() {
    var _this = this;
    var _a = useContext(GaesupWorldContext), moveTo = _a.moveTo, activeState = _a.activeState, cameraOption = _a.cameraOption, mode = _a.mode, block = _a.block;
    var dispatch = useContext(GaesupWorldDispatchContext);
    var _b = useState(true), isZoom = _b[0], setIsZoom = _b[1];
    var closeCamera = function (position) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            cameraOption.XDistance = position.x;
            cameraOption.YDistance = position.y;
            cameraOption.ZDistance = position.z;
            block.camera = true;
            dispatch({
                type: "update",
                payload: {
                    block: __assign({}, block),
                    cameraOption: __assign({}, cameraOption),
                },
            });
            return [2 /*return*/];
        });
    }); };
    var openCamera = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            block.camera = false;
            dispatch({
                type: "update",
                payload: {
                    block: __assign({}, block),
                },
            });
            return [2 /*return*/];
        });
    }); };
    var moveToCamera = function () { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!(mode.control === "orbit" && moveTo)) return [3 /*break*/, 2];
                    return [4 /*yield*/, moveTo(makeOrbitCameraPosition(activeState, cameraOption), activeState.position)];
                case 1:
                    _a.sent();
                    return [3 /*break*/, 4];
                case 2:
                    if (!(mode.control === "normal" && moveTo)) return [3 /*break*/, 4];
                    return [4 /*yield*/, moveTo(makeNormalCameraPosition(activeState, cameraOption), activeState.position)];
                case 3:
                    _a.sent();
                    _a.label = 4;
                case 4: return [2 /*return*/];
            }
        });
    }); };
    var setZoom = function (position, isZoom) { return __awaiter(_this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    setIsZoom(isZoom);
                    return [4 /*yield*/, closeCamera(position)];
                case 1:
                    _a.sent();
                    return [4 /*yield*/, moveToCamera()];
                case 2:
                    _a.sent();
                    return [4 /*yield*/, openCamera()];
                case 3:
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    }); };
    return {
        setZoom: setZoom,
        openCamera: openCamera,
        closeCamera: closeCamera,
        isZoom: isZoom,
        setIsZoom: setIsZoom,
        moveToCamera: moveToCamera,
    };
}
export function ZoomButton(props) {
    var _this = this;
    var _a = useZoom(), setZoom = _a.setZoom, isZoom = _a.isZoom;
    return (_jsx("div", { className: "zoomButton", style: __assign({}, props.zoomButtonStyle), onClick: function () { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, setZoom(props.position, isZoom)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        }); }, children: props.children }));
}
