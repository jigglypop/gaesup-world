...
블루프린트는 장기적으로 “월드에 놓이는 물체 데이터”만 뜻하지 않습니다. 이 프로젝트에서는 게임 제작자가 코드 수정 없이 편집하고 재사용해야 하는 설계 단위를 블루프린트로 봅니다.

우선 범위:

- 캐릭터, 차량, 오브젝트 같은 스폰 가능한 엔티티 정의
- NPC 성격, 역할, 기본 행동 규칙
- 자동 캐릭터 행동 에이전트의 상태, 조건, 액션 흐름
- 사건, 퀘스트, 대화, 보상처럼 트리거 기반으로 실행되는 게임플레이 흐름

범위 밖:

- 매 프레임 실행되는 카메라, 물리, 렌더링 계산
- React 컴포넌트 자체
- 특정 씬의 일회성 임시 상태
- 저장 파일 전체를 그대로 대체하는 포맷

즉, 블루프린트는 런타임 시스템 그 자체가 아니라 런타임 시스템이 읽어 실행할 수 있는 설계 데이터입니다.

## 설계 방향

블루프린트는 세 층으로 나누는 것이 좋습니다.
...
현재 구현은 `Entity Blueprint` 중심이고, `GameplayEventPanel`과 `npcStore` 쪽에 `Behavior Blueprint`, `Gameplay Event Blueprint`로 확장될 수 있는 조각이 이미 있습니다. 따라서 새 시스템을 별도로 만들기보다 기존 블루프린트 편집 경험과 이벤트/NPC 모델을 연결하는 방향이 자연스럽습니다.
