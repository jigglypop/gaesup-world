...
이 장은 뇌/LLM 응용을 위한 **유효 모델 문서**다. 미시 물리 규약은 `docs/경로적분.md` 최신 정본을 따른다.
...
본 장의 D1($\Phi_\text{supp}=-\log P_\text{selected}$)은 인지/추론 경로 선택을 표현하는 정보론적 보조 포텐셜이며, 미시 라그랑지안의 대체 정의가 아니다.
...
이로써, 뇌·의식·LLM 시스템에서도 CE 클라루스장 이론의
