...
- CE 공리(A1–A5), 정의(D1), 가설(H1)을 어떤 방식으로 여기에 결합할 수 있는지 명확히 한 뒤  
...
이 장은 표준 우주론의 수학 구조(FLRW, 프리드만 방정식)를 전제로 두고, CE 공리(A1–A5), 정의(D1), 가설(H1)을 덧붙여 암흑 에너지 성분을 해석하는 현상론적 모형을 정리한다.
...
- **3장**: 이 장에서 사용하는 CE 공리(A1–A5, D1, H1)의 역할  
...
이 장은 우주론 응용 계층 문서이며, 미시 규약은 `docs/경로적분.md` 최신 정본을 고정 입력으로 사용한다.
...
따라서 본 장의 D1($\Phi_\text{supp}=-\log P_\text{selected}$)과 $S(a),\mu(a)$는 관측 함수형을 다루는 **유효 기술**이며, 미시 라그랑지안을 다시 정의하지 않는다.
...
또한 동일 실행에서 베이스라인(ΛCDM, `mu=1`)의 `holdout chi2`를 함께 계산하며, `fsigma8_delta_chi2_holdout = chi2_holdout(model) - chi2_holdout(lcdm)`를 출력한다. 이 값이 음수이면(더 작으면) 해당 데이터 집합에 대해 CE 성장 보정이 ΛCDM 대비 적합도를 개선한 것으로 해석한다.
...
CE를 얹기 전에, 표준 우주론에서 사용하는 기본 수학 구조를 정리한다.  
...
CE 이론은 이 수치를 “처음부터 예측”한다고 주장하지 않고,  
다음과 같은 질문에 답하려고 한다.
...
- **H1**:  
  - 우주 전체의 “연산 복잡도 밀도”를 낮추는 방향으로  
    클라루스장이 작용하며, 그 결과로 **가속 팽창과 같은 전역 동역학**이 유도될 수 있다는 가정.
...
여기서 $\rho_\text{supp}$는 클라루스장에 기인하는 평균 에너지 밀도로,
...
### 4.2 $\Lambda$의 “미세 조정 문제”와 CE의 역할
...
의 형태로 합쳐진 뒤,  
클라루스장의 동역학에 의해 **대부분이 곡률 모드로 “재분배”되고**,  
우리가 큰 스케일에서 보는 것은 그 중 **작은 잔여분**이라는 해석이 가능하다.

여기서 핵심은,

- CE가 우주 상수의 정확한 수치 값을 “예측한다”고 주장하는 것이 아니라,  
...
이를 통해 CE의 우주론적 파라미터(예: $\alpha_\Lambda$, $\alpha_C$)에  
상당히 강한 제약을 걸 수 있다.
...
- **한계**  
  ...
  - $\Lambda_\text{bare}$와 $E_\text{nonselected}$ 간의 상쇄(cancellation) 메커니즘은  
    여전히 열린 문제이며, CE는 완전한 해답이라기보다  
    “동역학적 재조정”이라는 프레임을 제공하는 수준이다.

- **향후 과제**  
  - 구체적인 양자장 모형에서 $E_\text{nonselected}$를 계산하고,  
    CE coupling과 함께 $\Lambda_\text{eff}$를 수치적으로 추정하는 작업.  
